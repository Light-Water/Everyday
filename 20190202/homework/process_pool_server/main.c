...
int main(int argc,char** argv)
{
    ...
    makeChild(pManage,createProcessNum);//创建子进程 child.c
    int socketfd;
    tcpInit(&socketfd,argv[1],argv[2]);
    struct epoll_event event,*evs;//用epoll监控
    ...
    while(1)
    {
        ...
        for(i=0;i<fdReadyNum;i++)
        {
            if(evs[i].data.fd==socketfd)
            {
                ...
                {
                    if(0==pManage[j].busy)//找不忙碌的子进程,把描述符发给它
                    {
            ...
            for(j=0;j<createProcessNum;j++)
            {
                if(pManage[j].fds==evs[i].data.fd)//谁回写通知
                {
                    pManage[j].busy=0;
                    read(pManage[i].fds,&flag,sizeof(char));    
